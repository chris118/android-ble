package com.hhit.hhble.widget.fresco;
...
import com.hhit.hhble.util.FileManager;
...
public class ImagePipelineConfigFactory {
    ...
    public static ImagePipelineConfig getImagePipelineConfig(Context context) {
        if (sImagePipelineConfig == null)
            sImagePipelineConfig = configureCaches(context, get());
    ...
    private static ImagePipelineConfig configureCaches(Context context, OkHttpClient okHttpClient) {
        ...
        File fresco = FileManager.getInstance().createFrescoDir();
        //小图片的磁盘配置
        DiskCacheConfig diskSmallCacheConfig = DiskCacheConfig.newBuilder(context)
                .setBaseDirectoryPath(fresco)//缓存图片基路径
...
//            .setResizeAndRotateEnabledForNetwork(boolean resizeAndRotateEnabledForNetwork)//调整和旋转是否支持网络图片
    ...
    private static class ConfigConstants {
        ...
        public static final int MAX_SMALL_DISK_CACHE_VERY_LOW_SIZE = 10 * ByteConstants.MB;//小图极低磁盘空间缓存的最大值(特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图)
        public static final int MAX_SMALL_DISK_CACHE_LOW_SIZE = 20 * ByteConstants.MB;//小图低磁盘空间缓存的最大值(特性：可将大量的小图放到额外放在另一个磁盘空间防止大图占用磁盘空间而删除了大量的小图)
        ...
        public static final int MAX_DISK_CACHE_VERY_LOW_SIZE = 20 * ByteConstants.MB;//默认图极低磁盘空间缓存的最大值
        public static final int MAX_DISK_CACHE_LOW_SIZE = 60 * ByteConstants.MB;//默认图低磁盘空间缓存的最大值
